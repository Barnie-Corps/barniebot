...
export async function initializeShopItems() {
    try {
        ...
        await db.query("INSERT INTO rpg_items (name, description, type, rarity, base_value, tradeable, stackable, max_stack) VALUES ?", [itemRows]);

        const itemMap = new Map<string, number>();
        const existingItemRows: any = await db.query("SELECT id, name FROM rpg_items");
        ...
        const eqRows = equipmentData
            .filter(eq => itemMap.has(eq.item_name))
            .map(eq => [
                itemMap.get(eq.item_name),
                eq.slot,
                eq.required_level || 1,
                eq.required_class || null,
                eq.strength_bonus || 0,
                eq.defense_bonus || 0,
                eq.agility_bonus || 0,
                eq.intelligence_bonus || 0,
                eq.luck_bonus || 0,
                0,
                eq.mp_bonus || 0,
                null
        ...
        if (eqRows.length) {
            await db.query("INSERT INTO rpg_equipment (item_id, slot, required_level, required_class, strength_bonus, defense_bonus, agility_bonus, intelligence_bonus, luck_bonus, hp_bonus, mp_bonus, special_effect) VALUES ?", [eqRows]);
        }
        ...
        const consRows = consumableData
            .filter(cons => itemMap.has(cons.item_name))
            .map(cons => [
                itemMap.get(cons.item_name),
                cons.effect_type,
                cons.effect_value,
                0,
                0
        ...
        if (consRows.length) {
            await db.query("INSERT INTO rpg_consumables (item_id, effect_type, effect_value, duration, cooldown) VALUES ?", [consRows]);
        }
...
export async function initializeRPGData() {
    try {
        await Promise.all([
            (async () => {
                const existing: any = await db.query("SELECT COUNT(*) as count FROM rpg_achievements");
                ...
                if (existing[0].count === 0) {
                    ...
                    await db.query("INSERT INTO rpg_achievements (name, description, category, requirement_type, requirement_value, reward_gold, reward_experience, icon) VALUES ?", [rows.map(r => [r.name, r.description, r.category, r.requirement_type, r.requirement_value, r.reward_gold, r.reward_experience, r.icon])]);
                    Log.info("Achievements initialized", { component: "RPG" });
                }
            })(),
            (async () => {
                const existing: any = await db.query("SELECT COUNT(*) as count FROM rpg_crafting_materials");
                ...
                if (existing[0].count === 0) {
                    ...
                    await db.query("INSERT INTO rpg_crafting_materials (name, description, rarity, stack_size, drop_rate, emoji) VALUES ?", [rows.map(r => [r.name, r.description, r.rarity, r.stack_size, r.drop_rate, r.emoji])]);
                    Log.info("Crafting materials initialized", { component: "RPG" });
                }
            })(),
            (async () => {
                const existing: any = await db.query("SELECT COUNT(*) as count FROM rpg_pets");
                ...
                if (existing[0].count === 0) {
                    ...
                    await db.query("INSERT INTO rpg_pets (name, description, rarity, base_price, strength_bonus, defense_bonus, agility_bonus, intelligence_bonus, luck_bonus, special_ability, emoji) VALUES ?", [rows.map(r => [r.name, r.description, r.rarity, r.base_price, r.strength_bonus, r.defense_bonus, r.agility_bonus, r.intelligence_bonus, r.luck_bonus, r.special_ability, r.emoji])]);
                    Log.info("Pets initialized", { component: "RPG" });
                }
            })(),
            (async () => {
                const existing: any = await db.query("SELECT COUNT(*) as count FROM rpg_dungeons");
                ...
                if (existing[0].count === 0) {
                    ...
                    await db.query("INSERT INTO rpg_dungeons (name, description, required_level, difficulty, stages, boss_name, reward_gold_min, reward_gold_max, reward_exp_min, reward_exp_max, cooldown) VALUES ?", [rows.map(r => [r.name, r.description, r.required_level, r.difficulty, r.stages, r.boss_name, r.reward_gold_min, r.reward_gold_max, r.reward_exp_min, r.reward_exp_max, r.cooldown])]);
