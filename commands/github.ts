...
export default {
    ...
    async execute(interaction: ChatInputCommandInteraction, lang: string) {
        let message: string = "¿Te interesa utilizar el código de BarnieBot completamente gratis y actualizado siempre? Tú tranquil@, yo te cubro :wink:.\nAntes de darte el link, te pido encarecidamente que leas con atención la licencia a la que está sujeta mi código y des el respectivo crédito a mi creador :). A continuación, te dejaré los links del repositorio y del archivo que contiene un mensaje que sería bueno que incluyeras en tu código :). Están listados de manera respectiva:";
        if (lang !== "es") {
