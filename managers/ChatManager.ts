...
export default class ChatManager extends EventEmitter {
    ...
    private async FilterContent(message: Message<true>, userLanguage: string): Promise<boolean> {
        ...
        if (ai.OllamaEnabled && process.env.OLLAMA_CHAT_FILTER === "true") {
            response = await ai.getSingleOllamaResponse(ai.OllamaModerationModel, [{ role: "system", content: `Return JSON only with keys: suspicious(boolean), risk(\"low\"|\"medium\"|\"high\"), summary(string), reason(string), recommended_actions(array of up to 1 from [\"warn\",\"timeout\",\"blacklist\"]), warning_message(optional string for warn action), action_duration_ms(optional number), confidence(number 0-1). Never output compound actions. Prefer notify for uncertain cases. Reserve kick/ban for high risk with clear malicious evidence. Use recent_cases only as context; do not recommend punitive actions if the current content appears benign. If current content is benign, set suspicious=false, risk=low, recommended_actions=[]. Use language: ${userLanguage} for summary, reason, and warning_message.` }, { role: "system", content: `Content to analyze: ${content}` }], undefined, "json");
        }
